...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    reg = self.reg
    # self.params['X'] = X
    # Forward through affine1_relu (hidden layer)
    hidden_layer, cache_hidden_layer = affine_relu_forward(X, W1, b1)
    # Forward through affine2 (before softmax)
    ...
    ############################################################################
    # Forward through softmax to get loss
    loss, dscores = softmax_loss(scores, y)
    loss += 0.5 * reg * np.sum(W1 * W1) + 0.5 * reg * np.sum(W2 * W2)

    # Backprob through affine2
    dx1, grads['W2'], grads['b2'] = affine_backward(dscores, cache_scores)
    grads['W2'] += reg * W2

    # Backprob through affine1_relu
    # grads['X'], grads['W1'], grads['b1'] = affine_relu_backward(dhl, cache1)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    if self.use_batchnorm:
      for i in xrange(self.num_layers - 1): # last layer doesnt have batchnorm
    ...
    if self.use_batchnorm:
      ...
      self.bn_params = [{'mode': 'train',
                         ...
                         'running_var': np.zeros(dims[i + 1])} for i in xrange(self.num_layers - 1)]
  ...
  def loss(self, X, y=None):
    ...
    # Simple using without batchnorm and dropout
    ...
    # Using without batchnorm but with dropout
    if not(self.use_batchnorm) and self.use_dropout:

      for i in xrange(1, self.num_layers):
        layer[i], cache_layer[i] = affine_relu_dropout_forward(layer[i - 1],
                                                       self.params['W%d' % i],
                                                       self.params['b%d' % i],
                                                       self.dropout_param)
      # Forward into last layer
    ...
    # Using batchnorm but not dropout
    ...
    # Using batchnorm and drop out
    if self.use_batchnorm and self.use_dropout:
      for i in xrange(1, self.num_layers):
        layer[i], cache_layer[i] = affine_norm_relu_dropout_forward(layer[i - 1],
                                                       self.params['W%d' % i],
                                                       self.params['b%d' % i],
                                                       self.params['gamma%d' % i],
                                                       self.params['beta%d' % i],
    ...
    if not(self.use_batchnorm) and not(self.use_dropout):
      ...
      dx = {}
      dx[self.num_layers-1], grads[WLast], grads[bLast] = affine_backward(dscores, cache_scores)
      grads[WLast] += self.reg * self.params[WLast]

      # Backprop into remaining layers
      for i in reversed(xrange(1, self.num_layers)):
        dx[i-1], grads['W%d' % i], grads['b%d' % i] = affine_relu_backward(dx[i], cache_layer[i])
        grads['W%d' % i] += self.reg * self.params['W%d' % i]

    # Using without batchnorm and with dropout
    if not (self.use_batchnorm) and self.use_dropout:
      # Forward through softmax to get loss
      loss, dscores = softmax_loss(scores, y)
      # add regularization loss:
      ...
      # Backprop into remaining layers
      for i in reversed(xrange(1, self.num_layers)):
        dx[i - 1], grads['W%d' % i], grads['b%d' % i] = affine_relu_dropout_backward(dx[i], cache_layer[i])
        grads['W%d' % i] += self.reg * self.params['W%d' % i]
    ...
    if self.use_batchnorm and not(self.use_dropout):
      ...
      # Backprop into remaining layers
      for i in reversed(xrange(1, self.num_layers)):
        dx[i - 1], grads['W%d' % i], grads['b%d' % i], grads['gamma%d' % i], grads['beta%d' % i] \
          = affine_norm_relu_backward(dx[i], cache_layer[i])
        grads['W%d' % i] += self.reg * self.params['W%d' % i]
    ...
    if self.use_batchnorm and self.use_dropout:
      ...
      for i in reversed(xrange(1, self.num_layers)):
        dx[i - 1], grads['W%d' % i], grads['b%d' % i], grads['gamma%d' % i], grads['beta%d' % i] \
          = affine_norm_relu_dropout_backward(dx[i], cache_layer[i])
        grads['W%d' % i] += self.reg * self.params['W%d' % i]
...
def affine_norm_relu_dropout_forward(x, w, b, gamma, beta, bn_param, dropout_param):
  ...
  hnorm, hnorm_cache = batchnorm_forward(h, gamma, beta, bn_param)
  hnormrelu, relu_cache = relu_forward(hnorm)
  hnormrelu_dropout, dropout_cache = dropout_forward(hnormrelu, dropout_param)
  cache = (h_cache, hnorm_cache, relu_cache, dropout_cache)
  return hnormrelu_dropout, cache
...
def affine_norm_relu_dropout_backward(dout, cache):
    ...
    dhnormrelu_dropout = dropout_backward(dout, dropout_cache)
    dhnormrelu = relu_backward(dhnormrelu_dropout, relu_cache)
    dhnorm, dgamma, dbeta = batchnorm_backward_alt(dhnormrelu, hnorm_cache)
    dx, dw, db = affine_backward(dhnorm, h_cache)
...
def affine_relu_dropout_forward(x, w, b, dropout_param):
  ...
  h, h_cache = affine_forward(x, w, b)
  hrelu, relu_cache = relu_forward(h)
  hrelu_dropout, dropout_cache = dropout_forward(hrelu, dropout_param)
  cache = (h_cache, relu_cache, dropout_cache)
  return hrelu_dropout, cache
...
def affine_relu_dropout_backward(dout, cache):
    ...
    dhrelu_dropout = dropout_backward(dout, dropout_cache)
    dhrelu = relu_backward(dhrelu_dropout, relu_cache)
    dx, dw, db = affine_backward(dhrelu, h_cache)
